...
Local development may use `DB_HOST`, `DB_PORT`, `DB_USERNAME`, `DB_PASSWORD`, and `DB_NAME`. Railway uses `DATABASE_URL`. Also configure `OPEN_ROUTER_API_KEY`, Pixazo, R2 credentials/bucket, `FRONTEND_URL=https://story-hop.ru`, `BACKEND_URL=https://api.story-hop.ru`, `ALLOWED_ORIGINS=https://story-hop.ru`, `SESSION_COOKIE_DOMAIN=.story-hop.ru`, `HEALTH_DIAGNOSTICS_TOKEN`, and `WORKER_ENABLED=true`. Speaking transcription defaults to `OPENROUTER_STT_MODEL=mistralai/voxtral-small-24b-2507-stt` and uses `OPENROUTER_STT_FALLBACK_MODEL=openai/gpt-transcribe` only for an unavailable or invalid primary response. Production startup validates these values and rejects `DB_SYNCHRONIZE=true`.
